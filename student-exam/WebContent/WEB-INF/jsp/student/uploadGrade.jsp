...
<br>
 <hr style="clear:both;">
    ...
    <jsp:include page="/WEB-INF/jsp/common/add.jsp"></jsp:include>
	...
	<script src="${APP_PATH }/script/docs.min.js"></script>
	  ...
	  <script src="${APP_PATH }/jquery/jquery.pagination.js"></script>
       ...
       function queryPage(page_Index){
    	   ...
    	   $.ajax({
    		  url:"${APP_PATH}/student/queryBiaoStudentInfo.do",
    		  type:"POST",
    		  data:ObjParam,
    		  beforeSend:function(){
    			...
    			if(result.success){
    				...
    				$.each(page.datas,function(i,n){
    					content+=' <tr>';
       					...
       					content+='  <td>'+(n.pid == 1?"英语考试等级 "+n.type+" ":(n.pid == 6?"计算机二级考试科目 "+n.type+" ":"全国普通话考试 "))+'</td>';
       					content+='  <td>'+n.cardid+'</td>';
       					content+='  <td>'+n.baotime+'</td>';
       					content+='  <td>'+(n.mark == null?"还未录入成绩":n.mark)+'</td>';
       					content+='  <td>';      					
           				content +='<button type="button" onclick="addGrade('+n.id+')" class="btn btn-primary btn-xs"><i class=" glyphicon glyphicon-pencil"></i></button>';        	
       ...
       function deleteProDef(id,name){
    		...
    		layer.confirm("你确认要撤销"+name+"考试资格吗？",{icon: 3, title:'提示'}, function(cindex){
				layer.close(cindex);
				$.ajax({
	    			url:"${APP_PATH}/student/deletekaoshi.do",
	    			data:{
	    				"id":id
	    			},
	    			type:"POST",
	    			success:function(result){
	    				if(result.success){
	    					layer.msg("撤销成功", {time:1000, icon:5, shift:6});
	    					queryPage(0);
	    				}else{
	    					layer.msg("撤销失败", {time:1000, icon:5, shift:6});
	    				}
	    			}
	    		});
			}, function(cindex){
       ...
       function addGrade(id){
       	...
       	$("#upBtn").click(function(){
	    	...
	    	var time = $("#time"); 	   		    	
	    	if($.trim(mark.val()) == ""){   		    		
	    		layer.msg("分数必须填写", {time:1000, icon:5, shift:6}, function(){
	    			mark.val("");
	    			mark.focus();
	    		});    		
	    		return false;
       		...
       		$.ajax({
    			url:"${APP_PATH}/student/upStudentGrade.do",   			   		
    			data:{
    				"id":$("#id").val(),
    				mark:mark.val(),
    				time:time.val()
